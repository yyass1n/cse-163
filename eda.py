...
def text_to_float(df, column):
    """
    Converts the specified columns in the football dataset from string to float.
    """
    for col in column:
...
def possession_vs_outcome(df):
    ...
    fig = px.box(
        ...
        labels={'Outcome': 'Match Outcome', 'Ball_Possession_Home': 'Ball Possession Percentage'}
    )
    fig.show()
...
def possession_vs_goals(df):
    ...
    fig2 = px.density_heatmap(
        ...
        labels={'Ball_Possession_Home': 'Ball Possession Percentage', 'home_score': 'Goals Scored'}
    )
    fig2.show()
...
def main():
    ...
    print('Premier league data from 2018/2019 to 2021/2022 shape:', epl_df.shape)

    epl_df = transform_possession(epl_df)

    cols_of_interest = ['Ball_Possession_Home', 'Ball_Possession_Host', 'home_score', 'away_score',
                        'Shots_on_Goal_Home', 'Shots_on_Goal_Host', 'Dangerous_Attacks_Host', 
                        'Blocked_Shots_Home', 'Goalkeeper_Saves_Home', 'Goal_Attempts_Home', 
                        'Goal_Attempts_Host']
